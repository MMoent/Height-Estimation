...
def train(args):
    ...
    if restore_from is not None:
        ...
        save_dir_path = os.path.join('experiments', 'density_st_fusion_cons', 
                                        f'DENSITY_STCONS_{time_now}_{net_type.upper()}_EP{str(eps).zfill(3)}_BS{str(bs).zfill(2)}_LR{lr:.6f}'+ \
    ...
    for ep in range(current_ep+1, eps+1):
        ...
        for step, (aerial, ndsm, pano, sky, depth, ordinal) in loop:
            ...
            loop.set_description(f"DSC -- train on GPU {os.environ.get('CUDA_VISIBLE_DEVICES')}: epoch [{ep}/{eps}]")
        ...
        with torch.no_grad():
            ...
            for step, (aerial, ndsm, pano) in loop:
                ...
                loop.set_description(f"DSC -- val on GPU {os.environ.get('CUDA_VISIBLE_DEVICES')}: epoch [{ep}/{eps}]")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="train density with street cons")
